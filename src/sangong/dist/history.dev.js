...
var SangongHistoryLayer = cc.Layer.extend({
  bgLayer: null,
  dateInput: null,
  addressLabel: null,
  numPeriodLabel: null,
  ...
  ctor: function ctor() {
    ...
    for (var _index3 = 0; _index3 < 10; _index3++) {
      ...
      resultCards.attr({
        ...
        x: resultCards_width / 2 + resultCardsTitle.getContentSize().width + paddingX + _index3 * (resultCards_width + paddingX / 4),
    ...
    for (var _index4 = 0; _index4 < 3; _index4++) {
      ...
      cloneCard.attr({
        ...
        x: resultCards_width / 2 + cloneCardsTitle.getContentSize().width + paddingX + playerLabel.getContentSize().width + paddingX / 2 + _index4 * (resultCards_width + paddingX / 4),
        y: history1Content_height - numPeriodVal.getContentSize().height - paddingY - this.serial_num_panel.getContentSize().height - resultCards_height - paddingY
      });
    ...
    for (var _index5 = 0; _index5 < 3; _index5++) {
      ...
      cloneCard.attr({
        scaleX: resultCards_width / cloneCard.getContentSize().width,
        scaleY: resultCards_width / cloneCard.getContentSize().width,
        x: resultCards_width / 2 + cloneCardsTitle.getContentSize().width + paddingX + playerLabel.getContentSize().width + paddingX / 2 + resultCards_width * 3 + paddingX / 4 * 2 + paddingX / 2 + bankLabel.getContentSize().width + paddingX / 2 + _index5 * (resultCards_width + paddingX / 4),
        y: history1Content_height - numPeriodVal.getContentSize().height - paddingY - this.serial_num_panel.getContentSize().height - resultCards_height - paddingY
      });
      this.history1Content.addChild(cloneCard);
  ...
  gotoGame: function gotoGame(sender, type) {
    switch (type) {
      case ccui.Widget.TOUCH_ENDED:
        var gameScene = new SangongGameScene();
...
var SangongHistoryScene = cc.Scene.extend({
  onEnter: function onEnter() {
    this._super();

    var historyLayer = new SangongHistoryLayer();
