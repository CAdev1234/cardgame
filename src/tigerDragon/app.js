var LonghuGameLayer = cc.Layer.extend({
    ...
    ctor: function () {
        ...
        this.panelArea3.addChild(panelArea3_rate)
            ...
            onTouchBegan: (touch, event) => {
                ...
                if (this.enabledCoin.findIndex(this.findTrue) !== -1) {
                    ...
                    if (touch_x > 0 && touch_x < size.width && 
                        touch_y > (this.coinWrapSprite_height + this.betAmountBg_height - this.betAmountBg_height_delta + this.gamePanel_height / 2 + this.gamePanel_border / 2 + paddingY)) {
                        
                            if (this.panel1_DealedCoins.length === 0) {
                                this.panel1_ValRoundRect_Label = new cc.LabelTTF(coinVal, "Arial", 13)
                                this.panel1_ValRoundRect_Label.attr({
                                    fillStyle: cc.color(255, 255, 255),
                                })
                                ...
                                this.panel1_ValRoundRect.setPosition(cc.p(size.width / 2 - this.panel1_ValRoundRect.getContentSize().width / 2, paddingY / 2))
                        ...
                        touch_y < (this.coinWrapSprite_height + this.betAmountBg_height - this.betAmountBg_height_delta + this.gamePanel_height / 2 - this.gamePanel_border / 2 - paddingY)) {
                            ...
                            if (touch_x > 0 && touch_x < (size.width / 2 - paddingX)) {
                                ...
                                if (this.panel2_DealedCoins.length === 0) {
                                    ...
                                    this.panel2_ValRoundRect.setPosition(cc.p(size.width / 4 - this.panel2_ValRoundRect.getContentSize().width / 2, paddingY / 2))
                            ...
                            if (touch_x > (size.width / 2 + this.gamePanel_border / 2 + paddingX) && touch_x < size.width) {
                                if (this.panel3_DealedCoins.length === 0) {
                                    ...
                                    this.panel3_ValRoundRect.setPosition(cc.p(size.width / 4 - this.panel3_ValRoundRect.getContentSize().width / 2, paddingY / 2))
                    ...
                    this.betAmountTokenVal.setString(this.sumCoins(this.panel1_DealedCoins) + this.sumCoins(this.panel2_DealedCoins) + this.sumCoins(this.panel3_DealedCoins))
                }
    ...
    drawInterval: function () { 
        ...
        var countDrawSecond = setInterval(async () => {
            if (draw_second == 0) {
                ...
                this.panelArea2.setOpacity(50)

                await this.sleep(3000)

                await this.removeCards()
                await this.displayCardBack()
                
                this.panelArea1.setOpacity(255)
                this.panelArea2.setOpacity(255)
                this.betOpenInterval()
                return
            }
    ...
    displayCard: async function () {
        ...
        var resultCard_scaleY = this.cardWidth / this.cardBack[0].getContentSize().width * this.cardBack[0].getContentSize().height / this.resultCard[0].getContentSize().height
    ...
    removeCards: function () {
        ...
        for (let index = 0; index < this.resultCard.length; index++) {
    ...
    removeDealedCoins: function () {
        ...
        for (let index = 0; index < (this.panel1_DealedCoins.length + this.panel2_DealedCoins.length + this.panel3_DealedCoins.length); index++) {
