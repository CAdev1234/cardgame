...
var HomeLayer = cc.Layer.extend({
  ctor: function ctor() {
    ...
    var paddingY = size.width / 18.75; // background
    ...
    this.addChild(homeTryoutBtn, 9); // footer
    ...
    var gameItem_height = gameItem_width * gameItem_imgrate; // slogan defined

    var sloganSprite = new cc.Sprite(home_res.slogan_png);
    var sloganSprite_width = size.width / 7 * 4;
    var sloganSprite_height = sloganSprite_width / sloganSprite.getContentSize().width * sloganSprite.getContentSize().height; // guangbo defined

    var guangboSprite = new cc.Sprite(home_res.guangbo_png);
    var guangboSprite_width = size.width - paddingX * 2;
    var guangboSprite_height = guangboSprite_width / guangboSprite.getContentSize().width * guangboSprite.getContentSize().height; // niuniu button defined
    ...
    var niuniuBtn_height = niuniuBtn_width / niuniuBtn.getContentSize().width * niuniuBtn.getContentSize().height; // scrollview
    ...
    itemScroll.setPosition(cc.p(0, 0)); // itemScroll.setBackGroundImage(res.blackjack_jpg)

    this.addChild(itemScroll); // slogan attribute setting
    ...
    itemScroll.addChild(sloganSprite); // guangbo attribute setting
    ...
    itemScroll.addChild(guangboSprite); // niuniu item attribute setting
    ...
    itemScroll.addChild(niuniuBtn); // other game items
  ...
  homeTryout: function homeTryout(sender, type) {
    switch (type) {
  ...
  gotoNiuniu: function gotoNiuniu(sender, type) {
    switch (type) {
      case ccui.Widget.TOUCH_ENDED:
        cc.audioEngine.playEffect(home_res.game_item_mp3);
        console.log("gotoNiuniu"); // cc.director.popScene()

        cc.director.pushScene(new cc.TransitionFade(1.0, new NiuNiuGameScene()));
        break;
    }
  },
  gotoBaccarat: function gotoBaccarat(sender, type) {
    switch (type) {
      case ccui.Widget.TOUCH_ENDED:
        console.log("gotoBaccarat");
        cc.audioEngine.playEffect(home_res.game_item_mp3); // cc.director.popScene()

        cc.director.pushScene(new cc.TransitionFade(1.0, new BaccaratGameScene()));
        break;
    }
  },
  gotoLonghu: function gotoLonghu(sender, type) {
    switch (type) {
      case ccui.Widget.TOUCH_ENDED:
  ...
  gotoErbagang: function gotoErbagang(sender, type) {
    switch (type) {
      case ccui.Widget.TOUCH_ENDED:
  ...
  gotoGoldenFlower: function gotoGoldenFlower(sender, type) {
    switch (type) {
      case ccui.Widget.TOUCH_ENDED:
  ...
  gotoSangong: function gotoSangong(sender, type) {
    switch (type) {
      case ccui.Widget.TOUCH_ENDED:
  ...
  gotoSicBo: function gotoSicBo(sender, type) {
    switch (type) {
      case ccui.Widget.TOUCH_ENDED:
